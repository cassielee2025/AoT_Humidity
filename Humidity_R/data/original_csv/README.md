...
The files in this folder are the cleaned files csv files used in analysis. Other files originally downloaded from AoT Humidity were deleted.

# Files
* `data_no_nulls.csv` - original dataset with `NULL` manually removed from first and last row using a plain text editor (**Data not uploaded to GitHub**)
* `nodes.csv` - node ID and locations

## AoT Project Digest Readme
